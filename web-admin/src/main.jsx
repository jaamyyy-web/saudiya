...
const packs = [
  { grade: 'Grade 7', subject: 'Islamic Studies', title: 'Faith and Good Character', status: 'Published', medium: 'Arabic', questions: 38 },
  { grade: 'Grade 8', subject: 'Science', title: 'Chemical Reactions', status: 'Draft', medium: 'Arabic', questions: 27 },
  { grade: 'Grade 9', subject: 'Math', title: 'Linear Equations', status: 'Review', medium: 'Arabic', questions: 41 },
  { grade: 'Grade 7', subject: 'English', title: 'Reading Comprehension', status: 'Published', medium: 'Arabic Support', questions: 33 },
];

const reviewQueue = [
  { type: 'MCQ', subject: 'Science', issue: 'Needs better explanation', score: 82, status: 'Review' },
  { type: 'FIB', subject: 'Islamic Studies', issue: 'Clean output', score: 96, status: 'Approve' },
  { type: 'HOQ', subject: 'Math', issue: 'Check difficulty level', score: 74, status: 'Edit' },
  { type: 'TF', subject: 'Arabic', issue: 'Possible duplicate', score: 68, status: 'Review' },
];
    ...
    <div className="app-shell">
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        <div className="brand-row">
          ...
          <div>
            <div className="brand-title">SAD Admin</div>
            <div className="brand-subtitle">Saudi Education Control</div>
          </div>
    ...
    <section className="page-grid">
      <div className="hero-card wide">
        <div>
          ...
          <p>
            This panel is designed for desktop use and can be deployed separately without affecting the current Android app.
          </p>
          <div className="hero-actions">
            <button className="primary-button"><UploadCloud size={18} /> Upload PDF / Text</button>
      ...
      <div className="panel-card">
        <div className="panel-header">
          <h3>AI Quality Queue</h3>
          <span className="badge warning">74 pending</span>
        </div>
        <ReviewList />
      </div>
    </section>
      ...
      <div className="panel-card wide">
        <div className="panel-header">
          <div>
            ...
            <h3>Learning Pack Manager</h3>
          </div>
          <button className="primary-button"><UploadCloud size={18} /> New Upload</button>
        </div>
        <div className="form-grid">
          <AdminField label="Grade" value="Grade 7" />
          <AdminField label="Subject" value="Islamic Studies" />
          <AdminField label="Medium" value="Arabic" />
          <AdminField label="Difficulty" value="Mixed" />
        </div>
      </div>
  ...
  return (
    <section className="page-grid">
      <div className="panel-card wide">
        <div className="panel-header">
          <div>
            <p className="eyebrow">Gold Rubric</p>
            <h3>AI Quiz Review Queue</h3>
          </div>
          <button className="secondary-button"><Brain size={18} /> Generate More</button>
        </div>
        <ReviewList expanded />
      </div>
    </section>
  ...
  return (
    <section className="page-grid">
      <div className="panel-card wide">
        <div className="panel-header">
          <h3>Students</h3>
          <span className="badge success">Live analytics</span>
        </div>
          ...
          {students.map((student) => (
            <div className="student-row" key={student.name}>
              ...
              <div>
                ...
                <p>{student.grade} • {student.plan} • {student.streak} day streak</p>
              </div>
              <div className="progress-pill">{student.progress}%</div>
            </div>
          ))}
        </div>
      </div>
...
function PackTable() {
  return (
    <div className="table-wrap">
      <table>
        <thead>
          <tr><th>Grade</th><th>Subject</th><th>Learning Pack</th><th>Status</th><th>Questions</th></tr>
        </thead>
        <tbody>
          {packs.map((pack) => (
            <tr key={pack.title}>
              <td>{pack.grade}</td><td>{pack.subject}</td><td>{pack.title}</td>
              <td><span className={`badge ${pack.status.toLowerCase()}`}>{pack.status}</span></td>
              <td>{pack.questions}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="review-list">
      {reviewQueue.map((item) => (
        <div className="review-item" key={`${item.type}-${item.subject}`}>
          <div className="review-icon"><Brain size={20} /></div>
          <div>
            <strong>{item.subject} • {item.type}</strong>
            <p>{item.issue}</p>
          </div>
          <div className="score-badge">{item.score}%</div>
          {expanded && <button className="small-button"><CheckCircle2 size={15} /> Approve</button>}
        </div>
      ))}
    </div>
  );
...
function AdminField({ label, value }) {
  return <label className="admin-field"><span>{label}</span><input defaultValue={value} /></label>;
}
