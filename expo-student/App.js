...
const subjects = [
  ['islamic', 'الدراسات الإسلامية', 'moon', 86], ['arabic', 'لغتي الخالدة', 'language', 74], ['english', 'اللغة الإنجليزية', 'text', 61],
  ...
  ['health', 'التربية البدنية والدفاع عن النفس', 'fitness', 83], ['thinking', 'التفكير الناقد', 'bulb', 49],
].map(([id, ar, icon, score]) => ({ id, ar, icon, score }));

const grades = ['الصف السابع', 'الصف الثامن', 'الصف التاسع'];
const demoPacks = grades.flatMap((grade, g) => subjects.filter(s => s.id !== 'thinking' || grade === 'الصف التاسع').map((s, i) => ({
...
export default function App() {
  ...
  return <SafeAreaProvider><SafeAreaView style={styles.safe}><StatusBar style="dark" /><View style={styles.app}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
      {tab === 'home' && <Home student={displayStudent} packs={displayPacks} analytics={displayAnalytics} openPacks={openPacks} openTab={openTab} />}
      ...
      {tab === 'analytics' && <Analytics student={displayStudent} analytics={displayAnalytics} leaderboard={displayLeaderboard} openPacks={openPacks} />}
